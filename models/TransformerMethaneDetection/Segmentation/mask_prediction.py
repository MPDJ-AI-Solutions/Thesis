...
class MaskPrediction(nn.Module):
    def __init__(self, detr, freeze_detr=False):
        ...
        combiner_list = []
        num_channels = detr.backbone[0].num_channels  # taking list from RGB feature extractor
        num_pyd_layer = len(num_channels)
        for _layer in range(num_pyd_layer):
            in_channels = num_channels[_layer]
    ...
    def forward(self, samples: NestedTensor, samples1: NestedTensor, samples2: NestedTensor):
        if isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        features, pos, mf_out, rgb_feat, raw_feat = self.detr.backbone(
            samples.tensors, samples1.tensors, samples2.tensors
        )

        bs = features[-1].tensors.shape[0]

        src, mask = features[-1].decompose()
        mf_query, mf_mask = mf_out[-1].decompose()
        assert mask is not None
        src_proj = self.detr.input_proj(src)
        hs, memory = self.detr.transformer(src_proj, mask, self.detr.query_embed.weight, pos[-1], mf_query)

        # merge resnet features from raw and rgb data to create pyramid
        comb_features = []
        _layers = list(rgb_feat.keys())
        for idx, _layer in enumerate(_layers):
